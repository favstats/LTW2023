...
color_dat <- tibble(colors = c("#121212", "#b90e64", "#e00a1e", 
                               "#fecb2f", "#7aba2d", "#1a9fdd", 
                                "#a5defb", 
                               "#01acab", "#964B00", "#036b2c"),
                    party = c("CDU", "LINKE", "SPD", 
                              "FDP",  "GRÜNE", "AfD", 
                              "dieBasis", 
                              "FREIE WÄHLER", "HEIMAT", "KLIMALISTE"))
...
ggl_spend <- readRDS("../data/ggl_spend.rds") %>% filter(state == "Hessen")
...
```{r}
ggl_all <- readRDS("../data/ggl_all_Hessen.rds") %>% 
mutate(spendd = readr::parse_number(`Total Spend`)) %>% 
  arrange(desc(spendd)) %>% 
  select(-spendd)


ggl_all7 <- readRDS("../data/ggl_all7_Hessen.rds") %>% 
mutate(spendd = readr::parse_number(`Total Spend`)) %>% 
  arrange(desc(spendd)) %>% 
  select(-spendd)
